...
pub fn get_command_line_input() -> clap::ArgMatches { //possibly remove static lifetimes once I become clear what the lifetimes of App are
    ...
    .about("A terminal command line weather client with fine-grained options and a pretentious and possibly incorrect japanese name.")
    ...
    .arg(Arg::new("api")
        .long("api")
        .value_name("API")
        .about("Use specified api. Not all requested metrics are available for all apis.")
        .takes_value(true))
    ...
    .arg(Arg::new("graph")
        .short('g')
        .long("graph")
        .about("Show an ascii graph for the requested metrics")
        .value_name("METRICS")
        .takes_value(true))
    .arg(Arg::new("cache_duration")
        .long("cache-duration")
        .about("Specify the duration to cache previous results (in minutes)")
        .value_name("DURATION")
        .takes_value(true))
