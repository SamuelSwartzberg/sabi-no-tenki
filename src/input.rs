...
fn get_metric_vector(clap_parameter_option: Option<&str>) -> Vec<MetricType>{
    ...
    for metric_string in clap_parameter_option.map_or(std::iter::empty::<&str>(), |graph_string| graph_string.split(",")){
...
pub fn parse_matches_into_options(clap_matches: clap::ArgMatches) -> ProgOptions{
  let location_list = clap_matches.value_of("location").map_or(Vec::<String>::new(), |location_list_string| location_list_string.split(":").collect());
  let time_list = clap_matches.value_of("time").map_or(vec![chrono::Local::now()], |time_string| parse_time(time_string));
  let api = api::get_api(clap_matches.value_of("api").unwrap_or("not sure what we want to do when no api specified")); 
